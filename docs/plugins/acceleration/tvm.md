...
[here](../../tvm.md). The rest of this guide assumes TVM is installed at
`/opt/tvm` and its' libraries are at `/opt/tvm/build`.
...
After having installed TVM, vAccel and the TVM plugin you are ready to run
...
```
If vAccel and/or the TVM plugin are installed in a non-standard library path
