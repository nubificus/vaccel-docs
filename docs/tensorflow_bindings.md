...
To take advantage of vAccel's plugin system, we provide Tensorflow bindings
for simple inference operations. With the use of bindings, native Tensorflow
applications can leverage vAccel to run inference without modifications to
the original code.
...
After successfully executing the native application, you are ready to use it with
vAccel.
...
You can the use the bindings with `LD_PRELOAD`:
```sh
