...
**Note**: _For `aarch64` hosts, this process is slightly different (one would say easier!) as it assumes the Host system is a `L4T` distro. Just clone the repo and follow the [Build & install](jetson.md#build-and-install-jetson-inference) step_
